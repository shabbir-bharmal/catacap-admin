...
-   **Supavisor pool ceiling & dual-pool split:** `SUPABASE_DB_URL` connects through Supavisor (host `aws-…-pooler.supabase.com`). In **session mode** (port `5432`) the pooler caps total client connections at **15** per project; in **transaction mode** (port `6543`) the cap is higher but the codebase has not been audited for transaction-mode caveats (prepared statements, `LISTEN/NOTIFY`, session-level `SET` state, advisory locks). The combined `max` of every `pg.Pool` opened by this process MUST stay strictly below whichever Supavisor cap is in use, otherwise requests fail with `{"success":false,"message":"[CRASH:DB_SESSION] max clients reached in session mode – max clients are limited to pool_size: 15"}`. Both QA and production currently use session mode (5432). To prevent long-running cron jobs from starving HTTP requests, `server/src/db.ts` opens **two pools**: `pool` (HTTP, `max: 7`) and the named export `schedulerPool` (cron + manual triggers + background sweeps, `max: 3`) — combined ceiling of 10, leaving headroom for ad-hoc admin connections and `pg_dump`. All files in `server/src/scheduler/` MUST import `schedulerPool` (currently aliased as `pool` inside each scheduler file). Route handlers and `server/src/utils/` use the default `pool` export. If a future bump is needed, switch the connection string to port `6543` first rather than raising the combined `max` past `14`.
-   **Per-connection timeouts via `options=`:** `server/src/db.ts#buildConnectionString` appends `options=-c statement_timeout=20000 -c idle_in_transaction_session_timeout=30000 -c lock_timeout=10000 -c TimeZone=UTC -c application_name=catacap-admin-<role>` to the Supavisor URL so Postgres applies the limits during the startup handshake — *before* the first query runs. This avoids the documented `pool.on("connect")` race (an unawaited `SET` colliding with the first user query) that previously surfaced as the `Calling client.query() when the client is already executing a query` deprecation warning. The migration step in `testConnection()` temporarily drops `statement_timeout` to `3s` and restores it to `20s` (the new default) — never `0` — so a leaked timeout cannot disable the per-query cap on a recycled client.
-   **`/api/healthz/db`:** Returns `{ status, latencyMs, pools: [{ name, max, total, idle, waiting }], ping }` with HTTP 200 on success and 503 if `SELECT 1` doesn't return within 5s. Use this to distinguish "API up, DB wedged" from a hard outage without reading logs. `server/src/db.ts` additionally polls pool stats every 10s and logs a `[db-pool] pressure on …` warning whenever `waiting > 0` or `total >= max`.
-   **HTTP server timeouts:** `server/src/index.ts` sets `server.keepAliveTimeout = 65s`, `server.headersTimeout = 70s`, `server.requestTimeout = 30s`, and installs a per-request `res.setTimeout(25_000)` middleware that responds 504 if no handler reply arrives in time. The 25s middleware ceiling sits just above the 20s pool-acquire + 20s statement-timeout window so a single slow query still completes, but a request queued behind a stuck transaction cannot tie up resources indefinitely. The middleware **excludes** long-running endpoints whose path matches `/export(\b|/|-)`, `/uploads?(\b|/)`, `/backup(\b|/)`, `/admin/scheduler/…`, or which carry `?stream=true` — those rely on `server.requestTimeout` only. Add new long-running routes to `LONG_REQUEST_PATTERNS` rather than removing the middleware globally.
-   **Per-job statement_timeout overrides:** The 20s default is a hard cap for live HTTP work, but a few legitimately-long jobs lift it on their own client and restore `'20s'` before release: `runRetroactiveSweep` (`server/src/utils/matchingGrants.ts`) bumps to `5min`, and `runDailyCleanup` (`server/src/scheduler/dailyCleanup.ts`) bumps to `10min`. Both routes go through `schedulerPool`. If a new long job is added, follow the same pattern (`SET statement_timeout = '<window>'` after `connect()`, `SET statement_timeout = '20s'` in `finally` before `release()`); never set `'0'` or skip the reset, otherwise the lifted cap leaks onto whoever next acquires that pooled client. `server/src/utils/matchingGrants.ts` itself imports `schedulerPool as pool` so that both fire-and-forget `applyMatchGrants` calls (from `adminRecommendations.ts`, `adminPendingGrants.ts`, `adminOtherAssets.ts`) and `runRetroactiveSweep` (triggered from `adminMatching.ts` POST/PUT after the HTTP client is released) run on the scheduler pool, never starving the 7-slot HTTP pool.
